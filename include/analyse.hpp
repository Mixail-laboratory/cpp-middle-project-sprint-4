...
auto AnalyseFunctions(const std::vector<std::string> &files,
    ...
    std::vector<std::pair<function::Function, metric::MetricResults>> results;
    std::ranges::for_each(files, [&metric_extractor, &results](const std::string &iFileName) {
...
auto SplitByClasses(const auto &analysis) {
    auto with_classes = analysis | std::views::filter([](const auto &p) { return p.first.class_name.has_value(); });
    return with_classes | std::views::chunk_by([](const auto &lhs, const auto &rhs) {
...
auto SplitByFiles(const auto &analysis) {
    ...
    return analysis | std::views::chunk_by(
                          [](const auto &lhs, const auto &rhs) { return lhs.first.filename == rhs.first.filename; });
}
